...
//
// Arquitectura del pipeline concurrente:
//
//	┌─────────────┐    chunks     ┌──────────────────┐   records   ┌──────────────┐
//	│  FileReader  │ ─────────── ▶│  Worker Pool (N)  │ ──────────▶│  Aggregator  │
//	│  (goroutine) │   channel    │  goroutines       │  channel   │  (goroutine) │
//	└─────────────┘              └──────────────────┘             └──────────────┘
//
// El FileReader divide el CSV en bloques de líneas y los envía por el canal
// chunkCh. Cada worker toma un bloque, parsea y limpia cada fila, y envía los
// registros válidos al canal resultCh. El Aggregator recopila resultados y
// estadísticas.
package loader
...
// LoadFile carga el archivo CSV de forma concurrente y retorna los registros
// limpios junto con las estadísticas de la operación.
//
// Flujo:
//  1. fileReader lee el CSV y produce chunks → chunkCh
//  2. N workers consumen chunks, parsean/limpian → resultCh
//  3. aggregator recopila results y estadísticas
...
// ── fileReader ─────────────────────────────────────────────────────────────────
// Lee el CSV línea a línea y agrupa las líneas en chunks que envía al canal.
...
// ── worker ─────────────────────────────────────────────────────────────────────
// Cada worker toma chunks del chunkCh, parsea cada línea CSV y limpia el
// registro. Los resultados se envían al resultCh.
// Se ejecuta en su propia goroutine (N en total).
...
// ── aggregator ─────────────────────────────────────────────────────────────────
// Recopila todos los resultados de los workers y construye el slice final
// de registros limpios y las estadísticas de errores.
