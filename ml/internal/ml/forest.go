...
// DefaultForestConfig devuelve los hiperparámetros definidos en el
// diseño del modelo (sección 4 del informe).
...
// Train entrena el Random Forest en paralelo usando el patrón
// productor-consumidor con goroutines y channels:
//
//	taskCh  (productor → workers): índices de árboles a construir
//	treeCh  (workers → agregador): árboles entrenados
//
// Cada worker construye árboles de forma independiente sobre su propia
// muestra bootstrap, por lo que no existen condiciones de carrera:
// ningún estado mutable se comparte entre goroutines durante la
// construcción. La sincronización ocurre únicamente a través de los
// canales, siguiendo el principio de Go: "no comuniques compartiendo
// memoria; comparte memoria comunicando".
func Train(ds *Dataset, cfg ForestConfig) *RandomForest {
	...
	for w := 0; w < cfg.NumWorkers; w++ {
		...
		go func(workerID int) {
			defer wg.Done()
			// Cada worker tiene su PROPIO generador aleatorio: rand.Rand
			// no es seguro para uso concurrente, compartirlo crearía una
			// condición de carrera sobre su estado interno.
...
// bootstrapSample devuelve n índices muestreados con reemplazo del
// dataset (bagging). Cada árbol entrena sobre una muestra distinta,
// lo que reduce la varianza del ensamble.
...
// PredictBatch clasifica un dataset completo en paralelo, dividiendo
// las muestras entre NumWorkers goroutines. La inferencia es
// embarazosamente paralela: cada goroutine escribe en un rango
// disjunto del slice de salida, sin solapamiento.
